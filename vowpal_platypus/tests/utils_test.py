...
def test_split_object_dictionary():
    ...
    assert len(obj) == 2
    assert list(map(len, obj)) == [2, 2]
    assert sorted(sum([list(x.keys()) for x in obj], [])) == ['a', 'b', 'c', 'd']
    obj = split_object({'a': 1, 'b': 2, 'c': 3, 'd': 4, 'e': 5}, 2)
    assert isinstance(obj, list)
    assert len(obj) == 2
    assert list(map(len, obj)) == [3, 2]
    assert sorted(sum([list(x.keys()) for x in obj], [])) == ['a', 'b', 'c', 'd', 'e']
    obj = split_object({'a': 1, 'b': 2, 'c': 3, 'd': 4, 'e': 5, 'f': 6}, 2)
    assert len(obj) == 2
    assert list(map(len, obj)) == [3, 3]
    assert sorted(sum([list(x.keys()) for x in obj], [])) == ['a', 'b', 'c', 'd', 'e', 'f']
    ...
    assert len(obj) == 2
    assert list(map(len, obj)) == [2, 2]
    assert sum([list(map(len, x)) for x in [list(x.values()) for x in obj]], []) == [2, 2, 2, 2]
    with pytest.raises(ValueError) as excinfo:
